  ...
  return (
    <nav className="flex justify-between items-center px-10 py-3 sticky top-0 z-50 border-b backdrop-blur-md bg-bg-primary/50">
      <div
        ...
        className="font-playfair font-semibold text-gold text-lg italic hover:cursor-pointer"
        id="hero"
      >
        Pranjali{" "}
        <span className="not-italic font-semibold text-sage-dark ">Sengar</span>
      </div>
      ...
      <div className="hidden md:flex items-center gap-2">
        {navLinks.map((link) => (
          <a
            key={link.name}
            href={link.href}
            className="font-inter text-text-muted text-sm px-4 py-2 rounded-lg transition-all duration-300 ease-in-out hover:text-sage-dark hover:scale-110 hover:bg-sage-dark/10"
          >
            {link.name}
          </a>
        ))}
